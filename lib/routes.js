...
export default function(options) {
    ...
    router.get('*', (req, res) => {
        ...
        if (options.config.sentryDsn) {
            opts.sentryScript = '<script src="https://browser.sentry-cdn.com/4.0.4/bundle.min.js" crossorigin="anonymous"></script>';
            opts.sentryInit = `Sentry.init({ dsn: '${options.config.sentryDsn}' });`;

            if (req.user) {
                opts.sentryInit += dedent`

                    Sentry.configureScope(scope => {
                        scope.setUser({
                            'id': '${req.user.slackUserId}',
