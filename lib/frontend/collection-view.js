...
export default class CollectionView extends Backbone.View {
    ...
    _onSort() {
        ...
        this._childViews = this.collection.map(
            model => _.find(childViews, view => view.model === model));

        if (this._rendered) {
            this.$el.children().detach();
            this._childViews.forEach(view => this.$el.append(view.el));
        }
    }

    _onReset() {
        this._childViews.forEach(view => view.remove());
        this._childViews = [];
        this.collection.each(this._onAdd, this);
    }
    ...
    render() {
        this._rendered = true;
        this.$el.empty();
        this._childViews.forEach(view => this.$el.append(view.render().el));
