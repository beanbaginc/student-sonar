...
export default class ApplicationView extends Backbone.View {
    constructor(options) {
        ...
        this._template = `
            <div class="container-fluid content">
                ...
                <div class="col-xs-offset-4 col-sm-offset-3 col-md-offset-2 main">
        ...
        this._headerView = new HeaderView(childViewOptions);
        this._permissionDeniedView = new PermissionDeniedView(childViewOptions);
        this._rootView = new RootView(childViewOptions);
        this._sidebarView = new SidebarView(childViewOptions);
        this._studentProjectsView = new StudentProjectsListView(childViewOptions);

        if (loggedIn) {
            this._calendarView = new CalendarView(childViewOptions);
            this._myStatusReportsView = new MyStatusReportsView(childViewOptions);

            if (window.userType === 'mentor' || window.userType === 'instructor') {
                this._allStatusReportsView = new AllStatusReportsView(childViewOptions);
                this._userDetailViews = {};
            } else {
                this._allStatusReportsView = this._permissionDeniedView;
            }

            if (window.userType === 'mentor') {
                this._allStudentsListView = new AllStudentsListView(childViewOptions);
            } else {
                this._allStudentsListView = this._permissionDeniedView;
            }
        } else {
            this._calendarView = this._permissionDeniedView;
            this._myStatusReportsView = this._permissionDeniedView;
            this._allStatusReportsView = this._permissionDeniedView;
            this._allStudentsListView = this._permissionDeniedView;
        }
        ...
        this.listenTo(this.model, 'route:edit-status-report', function(dueDateId) {
            if (this.model.get('loggedIn')) {
                this._attachContent(new StatusReportEditorView({
                    model: this.model,
                    dueDateId: dueDateId
                }));
            } else {
        ...
        this.listenTo(this.model, 'route:view-status-report', function(reportId) {
            if (this.model.get('loggedIn')) {
                this._attachContent(new StatusReportReadingView({
                    model: this.model,
                    reportId: reportId
                }));
            } else {
        ...
        this.listenTo(this.model, 'route:my-status-reports', function() {
            this._attachContent(this._myStatusReportsView);
        });

        this.listenTo(this.model, 'route:all-status-reports', function() {
            this._attachContent(this._allStatusReportsView);
        });

        this.listenTo(this.model, 'route:projects', function() {
            this._attachContent(this._studentProjectsView);
        });

        this.listenTo(this.model, 'route:project', function(id) {
            this._attachContent(new StudentProjectView({
                model: this.model,
                taskID: id
            }));
        });
        ...
        this.listenTo(this.model, 'route:users', function() {
            this._attachContent(this._allStudentsListView);
        });
    ...
    _onRouteUser(username) {
        const isMentor = window.userType === 'mentor',
              isInstructor = window.userType === 'instructor';
        ...
        if (this._userDetailViews.hasOwnProperty(username)) {
            this._attachContent(this._userDetailViews[username]);
            return;
        }

        if (!this._ready) {
            this.listenToOnce(this.model, 'ready',
                              this._onRouteUser.bind(this, username));
            return;
        }

        const user = this.model.get('users').find(
            user => user.get('slack_username') === username);
        ...
        if (isMentor ||
            (isInstructor && intersectionExists(user.get('groups'),
                                                me.get('groups')))) {
