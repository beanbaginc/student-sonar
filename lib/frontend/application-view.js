import _ from 'underscore';
import Backbone from 'backbone';

import AllStatusReportsView from './all-status-reports-view';
import AllStudentsListView from './all-students-list-view';
import CalendarView from './calendar-view';
import FilteredCollection from './filtered-collection';
import HeaderView from './header-view';
import MyStatusReportsView from './my-status-reports-view';
import RootView from './root-view';
import SidebarView from './sidebar-view';
import StatusReportEditorView from './status-report-editor-view';
import StatusReportReadingView from './status-report-reading-view';
import StudentProjectsView from './student-projects-view';
import UserDetailView from './user-detail-view';


class PermissionDeniedView extends Backbone.View {
    constructor(options) {
        options.className = 'content-inner';
        super(options);
    }

    render() {
        this.$el.html(`
            <div class="page-header">
                <h1>Permission denied</h1>
            </div>
            `);

        if (this.model.get('loggedIn')) {
            $('<p/>')
                .text('You do not have access to this page. If you believe ' +
                      'this is an error, please contact your mentors.')
                .appendTo(this.$el);
        } else {
            $('<p/>')
                .text('To view this page, you need to be logged in.')
                .appendTo(this.$el);
        }
    }
}


/*
 * ApplicationView is a view that wraps the entire application. It shows a
 * header, and then shows different sub-views for each page.
 */
export default class ApplicationView extends Backbone.View {
    constructor(options) {
        super(options);

        this._template = `
            <div class="container-fluid content">
                <div class="col-xs-4 col-sm-3 col-md-2 sidebar">
                </div>
                <div class="col-xs-offset-4 col-sm-offset-3 col-md-offset-2 main">
                    <div id="spinner"><span class="fa fa-refresh fa-spin"></span></div>
                </div>
            </div>
            `;

        const loggedIn = this.model.get('loggedIn');

        this._headerView = new HeaderView({ model: this.model });
        this._permissionDeniedView = new PermissionDeniedView({ model: this.model });
        this._rootView = new RootView({ model: this.model });
        this._sidebarView = new SidebarView({ model: this.model });
        this._studentProjectsView = new StudentProjectsView({ model: this.model });

        if (loggedIn) {
            this._calendarView = new CalendarView({ model: this.model });
            this._myStatusReportsView = new MyStatusReportsView({ model: this.model });

            if (window.isAdmin) {
                this._allStatusReportsView = new AllStatusReportsView({ model: this.model });
                this._allStudentsListView = new AllStudentsListView({ model: this.model });
                this._userDetailViews = {};
            } else {
                this._allStatusReportsView = this._permissionDeniedView;
                this._allStudentsListView = this._permissionDeniedView;
            }
        } else {
            this._calendarView = this._permissionDeniedView;
            this._myStatusReportsView = this._permissionDeniedView;
            this._allStatusReportsView = this._permissionDeniedView;
            this._allStudentsListView = this._permissionDeniedView;
        }

        this._ready = this.model.get('ready');
        this.listenTo(this.model, 'ready', this._onReady);

        this.listenTo(this.model, 'route:calendar', function() {
            this._attachContent(this._calendarView);
        });

        this.listenTo(this.model, 'route:edit-status-report', function(dueDateId) {
            if (this.model.get('loggedIn')) {
                this._attachContent(new StatusReportEditorView({
                    model: this.model,
                    dueDateId: dueDateId
                }));
            } else {
                this._attachContent(this._permissionDeniedView);
            }
        });

        this.listenTo(this.model, 'route:view-status-report', function(reportId) {
            if (this.model.get('loggedIn')) {
                this._attachContent(new StatusReportReadingView({
                    model: this.model,
                    reportId: reportId
                }));
            } else {
                this._attachContent(this._permissionDeniedView);
            }
        });

        this.listenTo(this.model, 'route:my-status-reports', function() {
            this._attachContent(this._myStatusReportsView);
        });

        this.listenTo(this.model, 'route:all-status-reports', function() {
            this._attachContent(this._allStatusReportsView);
        });

        this.listenTo(this.model, 'route:projects', function() {
            this._attachContent(this._studentProjectsView);
        });

        this.listenTo(this.model, 'route:root', function() {
            this._attachContent(this._rootView);
        });

        this.listenTo(this.model, 'route:users', function() {
            this._attachContent(this._allStudentsListView);
        });

        this.listenTo(this.model, 'route:user', this._onRouteUser);
    }

    render() {
        this.$el
            .append(this._headerView.render().el)
            .append(this._template);

        this._$sidebar = this.$('.sidebar')
            .append(this._sidebarView.render().el);
        this._$main = this.$('.main');

        return this;
    }

    _onReady() {
        this._ready = true;

        if (this._activeView) {
            this._attachContent(this._activeView);
            this._activeView = undefined;
        }
    }

    _onRouteUser(username) {
        if (this.model.get('loggedIn') && window.isAdmin) {
            if (this._userDetailViews.hasOwnProperty(username)) {
                this._attachContent(this._userDetailViews[username]);
                return;
            }

            if (this._ready) {
                const user = this.model.get('users').find(
                    user => user.get('slack_username') === username);

                if (user) {
                    this._createAndAttachUserView(user);
                } else {
                    console.log('Could not find a User with slack username "%s"', username);
                }
            } else {
                this.listenToOnce(this.model, 'ready',
                                  this._onRouteUser.bind(this, username));
            }
        } else {
            this._attachContent(this._permissionDeniedView);
        }
    }

    _createAndAttachUserView(user) {
        const view = new UserDetailView({ model: user });
        this._userDetailViews[user.slack_username] = view;
        this._attachContent(view);
        return view;
    }

    _attachContent(view) {
        if (this._ready) {
            this._$main
                .children().detach().end()
                .append(view.el);

            view.render();
        } else {
            // Save the view to attach later, once everything is ready.
            this._activeView = view;
        }
    }
}
